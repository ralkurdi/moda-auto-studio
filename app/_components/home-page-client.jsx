...
export function HomePageClient({ featuredReview }) {
  ...
  return (
    <Screen title="MODA" right={<Ey>SSF · CA</Ey>}>
      ...
      {/* ── FEATURED REVIEW ──────────────────────────────────────────── */}
      {/* Hidden entirely when there are no real reviews to draw from. The
          server passes a random pick from the display=true pool on each
          request (page is force-dynamic) — refreshing the home rotates
          which review is featured. */}
      {featuredReview && (
        <section
        ...
        >
          ...
          >
            <Ey>&mdash; {featuredReview.author}</Ey>
            <button
